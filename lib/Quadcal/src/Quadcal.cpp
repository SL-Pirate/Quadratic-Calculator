...
EXTERNC QuadCal::QuadCal(double a, double b, double c){
    this->a = a;
    this->b = b;
    this->c = c;
    calDiscriminant();
...
//getters
EXTERNC double* QuadCal::getRoots(){
    double* pRoots = roots;
    return pRoots;
}
EXTERNC double QuadCal::getRoot(int i){
    return roots[i];
}
EXTERNC double QuadCal::getDiscriminant(){
    return discriminant;
}
EXTERNC std::string QuadCal::getEquation(){
    mkEqn();
    return equation;
}
EXTERNC std::string QuadCal::getNature(){
    mkNature();
    return nature;
}
EXTERNC std::string QuadCal::getRange(){
    mkRange();
    return range;
}
EXTERNC std::string QuadCal::getMinOrMax(){
    mkMinOrMax();
    return MinOrMax;
}
EXTERNC int QuadCal::getCode(){
    return code;
}

EXTERNC std::string QuadCal::format(double input){
    std::stringstream str;
    str << input;

    return str.str();
}
EXTERNC std::wstring QuadCal::format(double input, char y){
...
EXTERNC void QuadCal::calDiscriminant(){
    discriminant = ((pow(b, 2)) - (4*a*c));
}

EXTERNC void QuadCal::calRoots(){
    if (discriminant > 0){
...
EXTERNC std::wstring QuadCal::mkEqn(char y) {
    std::wstring aStr;
    std::wstring bStr;
    std::wstring cStr;
...
EXTERNC void QuadCal::mkMinOrMax(){
    if (a > 0){
        MinOrMax = "minimum";
    }
    else {
...
EXTERNC std::string QuadCal::handleRoots(double root){
    if (root > 0) {
        return " " + format(root);
    }
    else {
...
EXTERNC std::string QuadCal::Run(){
    switch (getCode()){
        case -2:
...
EXTERNC std::wstring QuadCal::getEquation(char y){
    return mkEqn(y);    
}

EXTERNC std::string QuadCal::Run(char y){
    switch (getCode()){
        case -2:
        case -1: return "Something went wrong\nError code: -1";
        break;
